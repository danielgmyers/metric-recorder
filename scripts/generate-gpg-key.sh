...
PROJECT_REALNAME="Metric Recorder Project"
PROJECT_EMAIL="dan+metricrecorder@danielgmyers.com"

TEMPDIR=$(mktemp -d)
echo "Working in ${TEMPDIR}..."

echo "Generating passphrase..."
PROJECT_GPG_PASSPHRASE=$(pwgen -s 16 1)

echo "Generating key..."
gpg --homedir $TEMPDIR --batch --gen-key <<FOO
    Key-Type: RSA
    Key-Length: 4096
    Name-Real: $PROJECT_REALNAME
    Name-Email: $PROJECT_EMAIL
    Expire-Date: 1y
    Passphrase: $PROJECT_GPG_PASSPHRASE
    %commit
    %echo done
FOO
...
PROJECT_GPG_KEY_ID=$(gpg --homedir $TEMPDIR --list-public-keys --with-colons --fingerprint | grep -B 1 $PROJECT_EMAIL | head -n 1 | cut -d ':' -f 10)
...
aws secretsmanager put-secret-value --secret-id gpg-signing-key-id --secret-string "$PROJECT_GPG_KEY_ID"

echo "Saving passphrase in secrets manager..."
aws secretsmanager put-secret-value --secret-id gpg-signing-key-passphrase --secret-string "$PROJECT_GPG_PASSPHRASE"
